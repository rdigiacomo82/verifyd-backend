...
app.add_middleware(
    ...
    allow_origins=[
        "https://vfvid.com",
        "https://www.vfvid.com",
        "https://paddlezlam.com",
        "https://www.paddlezlam.com",
        "http://localhost:5173",
        "http://localhost:3000",
        "http://127.0.0.1:5173",
    ],
...
# ─────────────────────────────────────────────
#  PaddleZlam Contact Form
# ─────────────────────────────────────────────
@app.post("/paddlezlam/contact")
async def paddlezlam_contact(request: Request):
    """
    Standalone contact endpoint for PaddleZlam.com.
    Does not touch VeriFYD detection, uploads, queues, workers, certificates,
    usage limits, or user verification logic.
    """
    import json
    import html
    ...
    # Basic spam honeypot support. If the frontend sends a hidden field named
    # website/company_url/honeypot and it is filled, silently accept but do nothing.
    honeypot = str(
        data.get("website")
        or data.get("company_url")
        or data.get("honeypot")
        or ""
    ).strip()
    if honeypot:
    ...
    organization = str(
        ...
        or data.get("school")
        or data.get("schoolOrganization")
        or data.get("company")
        or ""
    ).strip()
    message = str(
    ...
    resend_key = os.environ.get("PADDLEZLAM_RESEND_API_KEY") or os.environ.get("RESEND_API_KEY")
    to_email = os.environ.get("PADDLEZLAM_CONTACT_TO_EMAIL", "info@paddlezlam.com")
    from_email = os.environ.get("PADDLEZLAM_CONTACT_FROM_EMAIL", "PaddleZlam <noreply@paddlezlam.com>")

    if not resend_key:
        log.error("PaddleZlam contact: missing PADDLEZLAM_RESEND_API_KEY / RESEND_API_KEY")
    ...
    subject = f"New PaddleZlam Contact Request from {name}"

    html_body = f"""
    <h2>New PaddleZlam Contact Request</h2>
    <p><strong>Name:</strong> {safe_name}</p>
    <p><strong>Email:</strong> {safe_email}</p>
    <p><strong>Phone:</strong> {safe_phone or "Not provided"}</p>
    <p><strong>School / Organization:</strong> {safe_org or "Not provided"}</p>
    <p><strong>Message:</strong><br>{safe_msg}</p>
    <hr>
    <p style="font-size:12px;color:#666;">
      Sent from the PaddleZlam.com contact form.
    </p>
    """
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            log.info("PaddleZlam contact email sent: name=%s email=%s", name, email)
            return {
                "success": True,
        ...
        log.error("PaddleZlam contact Resend HTTP error: %s %s", e.code, detail)
        return JSONResponse(
            {"success": False, "error": "Email provider rejected the message", "detail": detail},
            status_code=502,
        )
    except Exception as e:
        log.exception("PaddleZlam contact send failed")
        return JSONResponse(
