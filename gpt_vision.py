...
def _build_physics_summary(ctx: dict) -> str:
    ...
    if content_type in ("cinematic", "action") and not is_person and skin_ratio < 0.15:
        hints.append(
            ...
            "TikTok in 2025-2026. Kling, Hailuo, Sora, and Veo produce photorealistic "
            "animal videos that fool most viewers. Your BASE ASSUMPTION should be AI "
            "— real wildlife videos filmed this cleanly are extremely rare.\n\n"
            "CRITICAL CHECKS FOR THIS VIDEO:\n"
            "  🦶 MOVEMENT BIOMECHANICS: Does the animal move with correct anatomy? "
            "Real large animals (moose, deer, elk, bear) have specific gait patterns — "
            "diagonal leg pairs move together, body bobs with each stride, hooves "
            "strike ground with visible impact. AI generators frequently get leg "
            "sequencing wrong, create floating legs, or produce unnaturally smooth "
            "gliding motion with no physical weight.\n"
            "  ❄️ SURFACE INTERACTION: If snow, mud, or grass is present — do the "
            "animal's feet sink, create tracks, or displace material? AI animals "
            "almost always float slightly above surfaces with zero physical trace.\n"
            "  💨 BREATH/VAPOR: In cold environments, real animals produce visible "
            "breath vapor on exhale. AI renders consistently miss this or render "
            "it incorrectly (wrong timing, wrong position relative to nostrils).\n"
            "  🌲 BACKGROUND QUALITY: Real wildlife footage looks like it was filmed "
            "— compression, handheld shake, imperfect focus. AI wildlife has "
            "cinematic-quality backgrounds that look rendered: too-perfect bokeh, "
            "repeating vegetation, unnaturally even lighting across the scene.\n"
            "  👁️ EYE AND FUR DETAIL: Zoom into the animal's eyes and fur. "
            "AI eyes are too perfect — symmetrical catchlights, no moisture or "
            "irregularity. AI fur has identical strand rendering without the natural "
            "clumping, matting, and directional variation of real animal coats.\n"
            "  📐 PROPORTIONS ACROSS FRAMES: Do the animal's proportions stay "
            "consistent? AI generators sometimes drift — legs lengthen slightly, "
            "body shape shifts between frames. Check antlers, ears, and limbs.\n\n"
            "SCORING: If the video looks 'cinematic' or 'too perfect' for wildlife: "
    ...
    # ── Ultra-high noise hint (static/dark real video) ──────
    # When sensor noise is extremely high (> 3000 Laplacian variance), this is
    # definitively a real camera — no AI video generator produces noise at this level.
    # AI renders: noise < 2500. Real cameras in low-light/dark: noise 3000-6000+.
    # This fires for ALL content types including static, which the sports hint skips.
    ...
    if _ultra_high_noise:
        hints.append(
            ...
            "This video has sensor noise levels that are only possible with a real camera "
            "in low-light or dark conditions. AI video generators cannot produce this "
            "level of sensor noise — this is definitively real camera footage.\n\n"
            "IMPORTANT SCORING GUIDANCE for this video:\n"
            "  → background_realism: The background may look dark, uniform, or simple "
            "because it is GENUINELY DARK — not because it is AI-rendered. "
            "Score 2 (real) unless you see impossible geometry or rendered textures.\n"
            "  → lighting_coherence: Consistent dim lighting in a dark real environment "
            "is normal. Score 2 (real) unless you see impossible light sources.\n"
            "  → color_naturalism: Low saturation and muted colors are expected in "
            "real dark/low-light footage. Score 2 (real) not 8 (AI).\n"
            "  → temporal_stability: A static or slow-moving real scene will have "
            "very consistent frames — this is normal, not an AI render artifact.\n\n"
            "DO NOT penalize this video for having consistent, dark, or simple visuals. "
            "The ultra-high noise is physical proof of a real camera sensor."
