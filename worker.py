...
def process_audio_upload_job(file_key: str, filename: str, email: str) -> dict:
    """Background job: retrieve standalone audio from R2/Redis, analyze it, and store result."""
    import os as _os
    import tempfile
    import hashlib
    from rq import get_current_job
...
def _audio_reasoning_for_ui(detail: dict, label: str) -> str:
    ...
    if label == "REAL":
        return f"Audio analysis found more natural recording characteristics than synthetic-audio indicators. {evidence_text}"
    if label == "AI":
        return f"Audio analysis found synthetic-media indicators in the track. {evidence_text}"
    return f"Audio analysis was mixed or inconclusive. {evidence_text}"
