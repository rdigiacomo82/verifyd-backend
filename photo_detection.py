...
def _build_photo_gpt_context(signal_score: int, signal_context: dict) -> dict:
    """
    # GEMINI_PHOTO_CONTEXT_FLAG_V2
    try:
        _sig_v2 = int(locals().get("signal_score", locals().get("photo_signal_score", 0)) or 0)

        for _ctx_name in ("detail", "signal_context", "photo_context", "detector_detail"):
            _ctx_obj = locals().get(_ctx_name)
            if isinstance(_ctx_obj, dict):
                _flat_v2 = _ctx_obj.get("flat_noise", _ctx_obj.get("flat_region_noise", None))
                _corr_v2 = _ctx_obj.get("chan_corr", _ctx_obj.get("channel_correlation", None))
                try:
                    _flat_num_v2 = float(_flat_v2) if _flat_v2 is not None else None
                except Exception:
                    _flat_num_v2 = None
                try:
                    _corr_num_v2 = float(_corr_v2) if _corr_v2 is not None else None
                except Exception:
                    _corr_num_v2 = None

                _risk_v2 = (
                    _sig_v2 >= 45 or
                    (_flat_num_v2 is not None and _flat_num_v2 <= 0.65 and _corr_num_v2 is not None and _corr_num_v2 >= 0.90)
                )

                if _risk_v2:
                    _ctx_obj["photo_ai_social_composite"] = True
                    _ctx_obj["gemini_photo_composite_guard"] = True
                    _ctx_obj["signal_score"] = _sig_v2
                    log.info("GEMINI_PHOTO_CONTEXT_FLAG_V2: enabled ctx=%s signal=%s flat=%s corr=%s", _ctx_name, _sig_v2, _flat_v2, _corr_v2)
    except Exception as _e:
        log.warning("GEMINI_PHOTO_CONTEXT_FLAG_V2 skipped: %s", _e)
...
def run_photo_detection(image_path: str) -> tuple:
    ...
    detail = {
        ...
        # GEMINI_PHOTO_COMPOSITE_FINAL_GUARD_V2
    try:
        _detail_v2 = detail if isinstance(detail, dict) else {}

        _gpt_result_v2 = locals().get("gpt_result", {}) or {}
        if not isinstance(_gpt_result_v2, dict):
            _gpt_result_v2 = {}

        _gpt_v2 = int(
            locals().get("gpt_score",
                locals().get("gpt_ai_score",
                    _gpt_result_v2.get("ai_probability", _detail_v2.get("gpt_ai_score", 50))
                )
            ) or 50
        )
        _sig_v2 = int(
            locals().get("signal_score",
                locals().get("signal_ai_score", _detail_v2.get("signal_ai_score", 0))
            ) or 0
        )

        _scores_v2 = _gpt_result_v2.get("scores", _detail_v2.get("gpt_scores", {})) or {}
        if not isinstance(_scores_v2, dict):
            _scores_v2 = {}

        _flags_v2 = _gpt_result_v2.get("flags", _detail_v2.get("gpt_flags", [])) or []
        if not isinstance(_flags_v2, list):
            _flags_v2 = [str(_flags_v2)]
        _flags_text_v2 = " ".join(str(x).lower() for x in _flags_v2)

        _flat_v2 = _detail_v2.get("flat_noise", _detail_v2.get("flat_region_noise", None))
        _corr_v2 = _detail_v2.get("chan_corr", _detail_v2.get("channel_correlation", None))
        try:
            _flat_num_v2 = float(_flat_v2) if _flat_v2 is not None else None
        except Exception:
            _flat_num_v2 = None
        ...
        _visual_hits_v2 = sum(
            ...
            if int(_scores_v2.get(k, 0) or 0) >= 6
        )

        _gpt_ai_flag_v2 = (
            "generator_artifacts" in _flags_text_v2 or
            "scene_staging" in _flags_text_v2 or
            "viral_ai_reel_pattern" in _flags_text_v2 or
            "staged_social_reel" in _flags_text_v2 or
            "gemini_photo_composite_guard" in _flags_text_v2
        )
        ...
        _gpt_confident_v2 = _gpt_v2 >= 64 and (_gpt_ai_flag_v2 or _visual_hits_v2 >= 2)
        _forensic_plus_gpt_v2 = _strong_forensic_v2 and _gpt_v2 >= 34

        if _gpt_confident_v2 or _forensic_plus_gpt_v2:
            _old_combined_v2 = locals().get("combined", locals().get("combined_ai_score", _detail_v2.get("ai_score", 50)))
            try:
                _old_combined_v2 = float(_old_combined_v2)
            except Exception:
                _old_combined_v2 = 50.0

            combined = max(_old_combined_v2, 72 if _gpt_confident_v2 else 68)
            combined_ai_score = combined
            authenticity = max(0, 100 - int(round(combined)))
            label = "AI"
            mode = "gemini/photo composite guard"

            if isinstance(detail, dict):
                detail["ai_score"] = int(round(combined))
                detail["combined_ai_score"] = int(round(combined))
                detail["authenticity"] = authenticity
                detail["label"] = label
                detail["blend_mode"] = mode
                detail["gemini_photo_composite_guard"] = True
                detail["gpt_flags"] = list(dict.fromkeys(["gemini_photo_composite_guard"] + _flags_v2))
                detail["gpt_reasoning"] = (
                    "This still image shows indicators of an AI-generated or AI-edited photo composite. "
                    "The review found either GPT-visible staging/generator artifacts or forensic signals such as low natural PRNU "
                    "and high channel correlation. Realistic skin, background texture, or fake camera grain should not override "
                    "physically implausible inserted objects, animals, food, props, or scene elements."
                )

            log.info(
                "GEMINI_PHOTO_COMPOSITE_FINAL_GUARD_V2: combined %s->%s label=%s gpt=%s signal=%s flat=%s corr=%s flags=%s visual_hits=%s",
                _old_combined_v2, combined, label, _gpt_v2, _sig_v2, _flat_num_v2, _corr_num_v2, _flags_v2, _visual_hits_v2
            )
    except Exception as _e:
        log.warning("GEMINI_PHOTO_COMPOSITE_FINAL_GUARD_V2 skipped: %s", _e)
